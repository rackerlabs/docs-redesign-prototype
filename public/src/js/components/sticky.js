    ...
    return {
        ...
        controller: function ($scope, $element, $attrs) {
            ...
            $scope.checkPosition = function () {
                ...
                var affix = $scope.getState(scrollHeight, height, offsetTop, offsetBottom);
            ...
            $scope.getState = function (scrollHeight, height, offsetTop, offsetBottom) {
                var scrollTop    = $scope.scrollParent.scrollTop();
                var position     = $element.offset();
                var targetHeight = $scope.scrollParent.height();

                if (offsetTop !== null && $scope.affixed == 'top') return scrollTop < offsetTop ? 'top' : false;

                if ($scope.affixed == 'bottom') {
                    if (offsetTop !== null) return (scrollTop + $scope.unpin <= position.top) ? false : 'bottom';
